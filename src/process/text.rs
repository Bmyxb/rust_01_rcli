...
pub fn process_text_encrypt(
    ...
    let encrpted_text = cipher.encrypt(nounce.into(), buf.as_ref())?;
    let encoded = URL_SAFE_NO_PAD.encode(encrpted_text);
    Ok(encoded)
}
...
pub fn process_text_decrypt(
    ...
    let mut buf = Vec::new();
    reader.read_to_end(&mut buf)?;
    let decoded = URL_SAFE_NO_PAD.decode(buf);
    let buf = decoded?;
    let cipher : ChaCha20Poly1305 = ChaCha20Poly1305::new(key.into());
    let decrpted_text = cipher.decrypt(nounce.into(), buf.as_ref())?;
    Ok(String::from_utf8(decrpted_text)?)
}

pub fn process_text_nounce_generate() -> Vec<u8> {
    ChaCha20Poly1305::generate_nonce(OsRng).to_vec()
}
